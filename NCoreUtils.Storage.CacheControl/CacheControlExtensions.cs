...
namespace NCoreUtils.Storage
{
    ...
    {
        public static Task SetCacheControlAsync(this IStorageItem item, TimeSpan cacheDuration, CancellationToken cancellationToken = default(CancellationToken))
        {
            var provider = item.StorageRoot.StorageProvider;
            if (provider.Features.TryGetFeature(out ICacheControlFeature feature))
            {
                return feature.UpdateCacheControlAsync(item, cacheDuration, cancellationToken);
            }
