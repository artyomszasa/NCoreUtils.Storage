    ...
    public class FakeGoogleClient : StorageClient
    {
        ...
        sealed class FakePagedAsyncEnumerable<TResponse, TResource> : PagedAsyncEnumerable<TResponse, TResource>
        {
            ...
            public override IAsyncEnumerable<TResponse> AsRawResponses()
                => _responses.ToAsyncEnumerable();

            public override IAsyncEnumerator<TResource> GetAsyncEnumerator(CancellationToken _)
                => _responses.SelectMany(_extract).ToAsyncEnumerable().GetAsyncEnumerator();
